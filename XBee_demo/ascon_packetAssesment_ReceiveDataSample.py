...
def main():
    ...
    try:
        ...
        def data_receive_callback(xbee_message):
            ...
            received_packets = int.from_bytes(decrypted_message[:4], 'big')
            integer_value = int.from_bytes(decrypted_message[4:], 'big')
            print(f"Message #{message_count}: From {xbee_message.remote_device.get_64bit_addr()} >> {integer_value}")
            total_bytes_received += (len(decrypted_message) / 8)
